...
- Wrappers: simple containing divs, no decorative purposes, functional only
...
Reference products: Linear, Raycast, Stripe, GitHub. They do not try to grab attention. They work.

## Hard No — Absolute Prohibitions
...
- No alignment that creates dead space to look expensive
...
No headline blocks with decorative subheaders:
...
- `<small>` headers are banned
- Rounded `span`s are banned
- Colors trending toward blue — banned. Dark muted colors are best.

No decorative note cards:
...
## Specifically Banned Implementation Patterns

These are concrete patterns observed in AI-generated UI that are explicitly prohibited:

- Border radii in the 20px to 32px range across everything (12px everywhere is also too much)
...
- Mixed alignment where some content hugs the left edge and some floats center-ish
...
If a UI choice feels like a default AI UI move, ban it and pick the harder, cleaner option. Colors stay calm, not fight.

## Color Selection Priority

1. **Highest priority**: Use existing colors from the user's project (search files to find them)
2. If no project palette exists, pick from a predefined palette below
3. Never invent random color combinations unless explicitly requested

Select randomly from available palettes — do not always pick the first one.
